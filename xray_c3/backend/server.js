...
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://postgres:your_password@localhost:5432/xray_viewer',
});
...
app.get('/api/patients/:name/:dob', async (req, res) => {
  try {
    ...
    const result = await pool.query('SELECT name, dob, gender, medical_history FROM patients WHERE name = $1 AND dob = $2', [name, dob]);
...
app.get('/api/patients/:name/:dob/xray', async (req, res) => {
  try {
    const { name, dob } = req.params;
    const result = await pool.query('SELECT xray_image FROM patients WHERE name = $1 AND dob = $2', [name, dob]);
    if (result.rows.length === 0) {
      res.status(404).json({ message: 'Patient not found' });
    } else {
      res.contentType('image/jpeg');
      res.send(result.rows[0].xray_image);
    }
  } catch (err) {
...
app.post('/api/patients', upload.single('xrayImage'), async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'INSERT INTO patients (name, dob, gender, medical_history, xray_image) VALUES ($1, $2, $3, $4, $5) RETURNING name, dob',
      [name, dob, gender, medicalHistory, xrayImage]
    );
    res.status(201).json({ message: 'Patient created successfully', name: result.rows[0].name, dob: result.rows[0].dob });
  } catch (err) {
