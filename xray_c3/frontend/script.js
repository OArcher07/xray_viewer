document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchPatientData(name, dob) {
        try {
            const response = await fetch(`http://localhost:3000/api/patients/${encodeURIComponent(name)}/${encodeURIComponent(dob)}`);
            if (!response.ok) {
    ...
    async function fetchXrayImage(name, dob) {
        try {
            const response = await fetch(`http://localhost:3000/api/patients/${encodeURIComponent(name)}/${encodeURIComponent(dob)}/xray`);
            if (!response.ok) {
